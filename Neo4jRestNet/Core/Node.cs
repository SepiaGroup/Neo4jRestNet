...
namespace Neo4jRestNet.Core
{
	public class Node : IGraphObject, IEquatable<Node>
	{
		...
		{
			return GetRootNode(DefaultConnectionName);
		}

		public static Node GetRootNode(string connectionName)
		{
			string response;
			var status = new StoreFactory().CreateNeo4jRestApi(connectionName).GetRoot(out response);
		...
		{
			return GetNode(DefaultConnectionName, nodeId);
		}

		public static Node GetNode(string connectionName, EncryptId nodeId)
		{
			string response;
			var status = new StoreFactory().CreateNeo4jRestApi(connectionName).GetNode((long)nodeId, out response);
			if (status != HttpStatusCode.OK)
			{
		...
		{
			return GetNode(DefaultConnectionName, indexName, key, value);
		}

		public static IEnumerable<Node> GetNode(Enum indexName, string key, object value)
		{
			return GetNode(DefaultConnectionName, indexName.ToString(), key, value);
		}
		...
		public static IEnumerable<Node> GetNode(string connectionName, string indexName, string key, object value)
		{
			string response;
			var status = new StoreFactory().CreateNeo4jRestApi(connectionName).GetNode(indexName, key, value, out response);
			if (status != HttpStatusCode.OK)
			{
		...
		public static IEnumerable<Node> GetNode(string connectionName, Enum indexName, string key, object value)
		{
			return GetNode(connectionName, indexName.ToString(), key, value);
		}

		public static IEnumerable<Node> GetNode(string connectionName, string indexName, Enum key, object value)
		{
			return GetNode(connectionName, indexName, key.ToString(), value);
		}

		public static IEnumerable<Node> GetNode(string connectionName, Enum indexName, Enum key, object value)
		{
		...
		{
			return GetNode(DefaultConnectionName, indexName.ToString(), searchQuery);
		}

		public static IEnumerable<Node> GetNode(string connectionName, string indexName, string searchQuery)
		{
			string response;
			var status =  new StoreFactory().CreateNeo4jRestApi(connectionName).GetNode(indexName, searchQuery, out response);
			if (status != HttpStatusCode.OK)
			{
				throw new Exception(string.Format("Index not found in (index:{0})", indexName));
			}
		...
		public static IEnumerable<Node> GetNode(string connectionName, Enum indexName, string searchQuery)
		{
		...
		public static Node CreateNode(string nodeType)
		{
			...
			return CreateNodeFromJson(DefaultConnectionName, properties.ToString());
		}
		...
		public static Node CreateNode(string connectionName, string nodeType)
		{
			var properties = new Properties();
			properties.SetProperty(NodeProperty.NodeType.ToString(), nodeType);
			return CreateNodeFromJson(connectionName, properties.ToString());
		}
		public static Node CreateNode(string connectionName, Enum nodeType)
		{
			return CreateNode(connectionName, nodeType.ToString());
		}
		...
		public static Node CreateNode(string connectionName, string nodeType, Properties properties)
		{
			properties.SetProperty(NodeProperty.NodeType.ToString(), nodeType);
			return CreateNodeFromJson(connectionName, properties.ToString());
		}

		public static Node CreateNode(string connectionName, Enum nodeType, Properties properties)
		{
			return CreateNode(connectionName, nodeType.ToString(), properties);
		}
		...
		public static Node CreateNode(string nodeType, IDictionary<string, object> properties)
		{
			...
			return CreateNodeFromJson(DefaultConnectionName, JObject.FromObject(properties).ToString(Formatting.None));
		}
		...
		public static Node CreateNode(string connectionName, string nodeType, IDictionary<string, object> properties)
		{
			properties.Add(NodeProperty.NodeType.ToString(), nodeType);
			return CreateNodeFromJson(connectionName, JObject.FromObject(properties).ToString(Formatting.None));
		}

		public static Node CreateNode(string connectionName, Enum nodeType, IDictionary<string, object> properties)
		{
			return CreateNode(connectionName, nodeType.ToString(), properties);
		}

		private static Node CreateNodeFromJson(string connectionName, string jsonProperties)
		{
			string response;
			var status = new StoreFactory().CreateNeo4jRestApi(connectionName).CreateNode(jsonProperties, out response);
        ...
        {
			var status = new StoreFactory().CreateNeo4jRestApi(_selfDbUrl).DeleteNode(Id);
		...
		private void LoadProperties(bool refresh)
		{
			...
			string response;
			var status = new StoreFactory().CreateNeo4jRestApi(_selfDbUrl).GetPropertiesOnNode((long)EncryptedId, out response);
			if (status != HttpStatusCode.OK)
			{
		...
		public void SaveProperties(Properties properties)
		{
			...
			var status = new StoreFactory().CreateNeo4jRestApi(_selfDbUrl).SetPropertiesOnNode((long)EncryptedId, properties.ToString());
			if (status != HttpStatusCode.NoContent)
			{
		...
		{
			string response;
			HttpStatusCode status = new StoreFactory().CreateNeo4jRestApi(DefaultConnectionName).GetRelationshipsOnNode((long)EncryptedId, direction, names, out response);
			if (status != HttpStatusCode.OK)
			{
		...
		{
			string response;
			HttpStatusCode status = new StoreFactory().CreateNeo4jRestApi(DefaultConnectionName).CreateRelationship( 
																	(long)EncryptedId, 
																	toNode.Self, 
																	relationshipType, 
		...
		{
			string response;
			var status = new StoreFactory().CreateNeo4jRestApi(DefaultConnectionName).Traverse((long)EncryptedId, order, uniqueness, relationships, pruneEvaluator, returnFilter, maxDepth, returnType, out response);
			if (status != HttpStatusCode.OK)
			{
		...
		{
			return AddNodeToIndex(DefaultConnectionName, nodeId, indexName, key, value);
		}
		
		public static Node AddNodeToIndex(long nodeId, Enum indexName, string key, object value)
		{
			return AddNodeToIndex(DefaultConnectionName, nodeId, indexName.ToString(), key, value);
		}

		public static Node AddNodeToIndex(long nodeId, string indexName, Enum key, object value)
		{
			return AddNodeToIndex(DefaultConnectionName, nodeId, indexName, key.ToString(), value);
		}

		public static Node AddNodeToIndex(long nodeId, Enum indexName, Enum key, object value)
		{
			return AddNodeToIndex(DefaultConnectionName, nodeId, indexName.ToString(), key.ToString(), value);
		}

		public static Node AddNodeToIndex(string connectionName, long nodeId, Enum indexName, Enum key, object value)
		{
			return AddNodeToIndex(connectionName, nodeId, indexName.ToString(), key.ToString(), value);
		}

		public static Node AddNodeToIndex(string connectionName, long nodeId, string indexName, string key, object value)
		{
			string response;
			var status = new StoreFactory().CreateNeo4jRestApi(connectionName).AddNodeToIndex(nodeId, indexName, key, value, out response);
			if (status != HttpStatusCode.Created)
			{
		...
		{
			return RemoveNodeFromIndex(DefaultConnectionName, nodeId, indexName);
		}

		public static HttpStatusCode RemoveNodeFromIndex(long nodeId, Enum indexName)
		{
			return RemoveNodeFromIndex(DefaultConnectionName, nodeId, indexName.ToString());
		}

		public static HttpStatusCode RemoveNodeFromIndex(string connectionName, long nodeId, Enum indexName)
		{
			return RemoveNodeFromIndex(connectionName, nodeId, indexName.ToString());
		}

        public static HttpStatusCode RemoveNodeFromIndex(string connectionName, long nodeId, string indexName)
		{
			var status = new StoreFactory().CreateNeo4jRestApi(connectionName).RemoveNodeFromIndex(nodeId, indexName);
			if (status != HttpStatusCode.NoContent)
			{
		...
		{
			return RemoveNodeFromIndex(DefaultConnectionName, nodeId, indexName, key);
		}
		
		public static HttpStatusCode RemoveNodeFromIndex(long nodeId, Enum indexName, string key)
		{
			return RemoveNodeFromIndex(DefaultConnectionName, nodeId, indexName.ToString(), key);
		}

		public static HttpStatusCode RemoveNodeFromIndex(long nodeId, string indexName, Enum key)
		{
			return RemoveNodeFromIndex(DefaultConnectionName, nodeId, indexName, key.ToString());
		}

		public static HttpStatusCode RemoveNodeFromIndex(long nodeId, Enum indexName, Enum key)
		{
			return RemoveNodeFromIndex(DefaultConnectionName, nodeId, indexName.ToString(), key.ToString());
		}
		
		public static HttpStatusCode RemoveNodeFromIndex(string connectionName, long nodeId, Enum indexName, Enum key)
		{
			return RemoveNodeFromIndex(connectionName, nodeId, indexName.ToString(), key.ToString());
		}

        public static HttpStatusCode RemoveNodeFromIndex(string connectionName, long nodeId, string indexName, string key)
		{
			var status = new StoreFactory().CreateNeo4jRestApi(connectionName).RemoveNodeFromIndex(nodeId, indexName, key);
			if (status != HttpStatusCode.NoContent)
			{
				throw new Exception(string.Format("Error remove node from index (node id:{0} index name:{1} http response:{2})", nodeId, indexName, status));
			}
		...
		{
			return RemoveNodeFromIndex(DefaultConnectionName, nodeId, indexName, key, value);
		}

		public static HttpStatusCode RemoveNodeFromIndex(long nodeId, Enum indexName, string key, object value)
		{
			return RemoveNodeFromIndex(DefaultConnectionName, nodeId, indexName.ToString(), key, value);
		}
        ...
        public static HttpStatusCode RemoveNodeFromIndex(string connectionName, long nodeId, string indexName, string key, object value)
		{
			var status = new StoreFactory().CreateNeo4jRestApi(connectionName).RemoveNodeFromIndex(nodeId, indexName, key, value);
			if (status != HttpStatusCode.NoContent)
			{
				throw new Exception(string.Format("Error remove node from index (node id:{0} index name:{1} http response:{2})", nodeId, indexName, status));
			}
